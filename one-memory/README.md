...
通用 Git 钩子，任何终端 Git、任何 Agent（Claude Code, Cursor, Aider 等）通用：

### 通用 Git 钩子 (Universal Git Hooks)
...
### 2. 规则记忆（规则轨，写入 rules.md）
- 遵循 `references/rules-memory.md`；
- **触发**：用户教导行为模式 / 纠正或指责 AI 行为 / 表达偏好实现方式，或 Agent 在构建/测试/调试/部署中发现项目知识时；
- **分流**：换个项目仍成立 → 海马仓 `rules.md`（one-ghfile 远端直写）；仅本项目有效 → `<项目根>/onememory/rules.md`；跨项目个人偏好走 `personal/preferences.md`；
- **边界**：只记"怎么做"（行为规则），不记"做了什么"（落入事实轨 tasks/ 案卷）；
- **治理**：单文件上限 150 行，超限同类合并；改动与代码同批提交，不参与门禁强校验；
- **读取**：全局规则每次会话先读；项目规则进入项目首次回复前读，作为项目级指令。
