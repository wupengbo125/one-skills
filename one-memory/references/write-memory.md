...
### Step 1 生成改动摘要
把摘要当 **commit subject** 写：一行，≤60 字，只放将来能 grep 到的关键字——动了哪个文件、命令、概念。
摘要只装"改了什么"；"为什么改"和"怎么查出来的"装在案卷里，行首会话 ID 就是案卷指针。
...
2. **事实轨 案卷**（摘要装不下的都进这里）：记背景诉求、排查过程、关键决策、代码结论、涉及产物。同一会话涉及多个完全不同的主题，用分隔符"---"分开写。  
文件名：`tasks/<会话ID>.md`
