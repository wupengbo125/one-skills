---
name: one-implement
description: "编写、修改、重构代码或开发功能时使用。架构者模式：主Agent只做设计与分发，严禁亲自写业务代码；派发Worker Sub-agent隔离写代码，派发双Reviewer Sub-agent严查Git Diff。"
...
用户提需求
   │
   ▼
1. 架构与边界（主 Agent：提取 Target & Non-Goals，严禁亲自改代码）
   │
   ▼
2. 隔离实现（派 Worker Sub-agent：全新独立上下文，戴锁编码）
   │
   ▼
3. 测试验证（Worker Sub-agent 或专用测试 Agent 运行单测）
   │
   ▼
4. 双轴审查（派 Reviewer A & B 两个全新 Sub-agent 并行查 Git Diff）
   ├── Reviewer A（Standards 轴）：查过度设计、投机抽象、坏味道
   └── Reviewer B（Scope 轴）：查范围蔓延、未要求改动、自作主张
   │
   ▼
...
2. **意图锚定**：
   - 从用户的一句话或文档中，提炼出清晰的 **Target（必做目标）** 与 **Non-Goals（严禁扩展项）**。
   - 不进行无意义的小盘问，迅速形成任务派发指令。
...
- **拒绝多层兜底**：不写“A 不行试 B，B 不行试 C”；直接用 A，挂了直接修 A。
...
Worker Sub-agent 完工后仅向主 Agent 返回简短执行总结（改了哪些文件、是否测试通过），主会话上下文保持绝对干净。
...
- 拿主 Agent 第 1 步的 Target 与 Non-Goals 当尺子：
...
1. **被打回**：主 Agent 重新唤醒 Worker Sub-agent，命令其删除多余改动，修完重新 Review。
2. **交付闸门**：
