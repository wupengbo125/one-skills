...
- 海马体记忆伴随模式 (Companion Mode): 遵循 one-free-me skill（遇未知先查 free-me，有硬核认知随手沉淀入库，知识库全中文，宁缺毋滥不记垃圾）
...
在任何对话与编码中全程伴随执行（遵循 `one-free-me` skill）：
* **伴随查**：遇到未知代号或私有服务，查 `system/aliases.md` 或执行 `free-me search "<关键词>"`。
* **伴随存**：跑通关键方法、排查深坑或确认资产位置时，随手写入 `one-hippocampus/` 并执行 `free-me sync "<相对路径>"`。
* **知识库全中文**：Skill 保持 `one-free-me`，海马体知识库文档 100% 采用中文命名。
