---
name: one-wiki
description: "Use when: 摄入文章/对话到 Wiki、查询与维护 OneWiki 知识库时使用。"
argument-hint: "ingest | query | lint"
---

# OneWiki V2 个人深度知识库

基于第一性原理打造的个人知识资产系统：**零 raw 垃圾倾倒，进库即精品，自包含且直接可用。**

知识库根路径：`$one_llmwiki_dir/`
...
* **入库与挂载**：
  ...
  3. 执行 Git 提交并推送：
     ```bash
     cd $one_llmwiki_dir && git add . && git commit -m "docs(wiki): add <domain>/<name>" && git push
     ```
...
### 2. query（分级地图寻路检索）
当用户查询某个概念或技术方案时：
1. **查总地图**：读 `INDEX.md`，根据意图锁定知识领域（如 `tech/`、`finance/` 等）；
