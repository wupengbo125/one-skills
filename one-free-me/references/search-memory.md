...
涉及给用户做**推荐 / 选择 / 建议**（美食、工具、穿搭、出行、购物、技术选型等）时，**AI 必须先查偏好再作答**：

- 读 `~/onespace/github/one-hippocampus/preference.md` 找相关偏好条目；
- 必要时刻读 `system/profile.md`（身份、画像、环境）；
- 用偏好**定制**推荐，不做泛泛推荐。

例：用户要推荐西安美食 → 先读 preference.md 的美食偏好（兴平"零点烫菜"、喜欢健康自制酱料），据此推荐并给地址；用户要选工具 → 先看偏好（极简主义、绿色免安装、优先成本）。
