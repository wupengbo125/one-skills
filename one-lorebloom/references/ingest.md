...
4. 读原文，先识别**核心**：这篇原文要传达的那个道理、方法或对象信息。所有抽取围绕核心；用来引出核心的场景与道具是**媒介**（如借乒乓球讲人生道理，乒乓球即媒介）。写入 `lorebloom/<领域>/`：
   ...
   - 正文只写知识：双链在相关内容处自然嵌入，相关链接统一放在「## 相关」标题下；正文不出现"跨域""双链""摄入"等元术语。
   - **属性归属正确**：每条信息只放在描述它的那个对象的页面（杨波的擅长放杨波页，药房的电话放药房页）；页面间可以关联引用，引用其他实体用 `[[双链]]` 指过去，不把对方的属性复制成自己的。
