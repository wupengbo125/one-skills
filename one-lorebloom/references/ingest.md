...
4. 读原文，写入 `lorebloom/<领域>/`：
   - **summaries**：一对一完整浓缩，保留原文全部要点、数据、步骤、结论，去掉网页导航/广告/重复；头部写 `> 源文件：[[raw/<领域>/<文件名>|查看原文]]`。
   - **concepts**：只给可复用的方法论、跨多篇资料出现的思维模型独立建页；单次观点、一时感悟留在 summary 里。
   - **entities**：外部对象——人物、工具、产品、地点、食材。
   - **projects**：我的项目——Carefree、Paseo插件等，记录状态、进展、待办。
   - 关联已有页面时增量追加；主动建立跨领域 `[[双链]]`。
