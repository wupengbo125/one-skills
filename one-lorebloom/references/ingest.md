...
4. 读原文，先识别**核心**：这篇原文要传达的那个道理、方法或对象信息。所有抽取围绕核心；用来引出核心的场景与道具是**媒介**（如借乒乓球讲人生道理，乒乓球即媒介）。写入 `lorebloom/<领域>/`：
   ...
   - **实体信息隔离**：每个 entity 页面只写该实体自身的属性；原文里并列出现的不同对象（如药房与医生）各自独立成页，不把一个对象的信息搬进另一个对象的页面。
   - **清单型原文**：原文由多条并列条目组成时（如资源清单、通讯录），每条独立建页；清单全貌由 summary 承载或靠领域 index 聚合，不在单条目页里塞入其他条目。
   - **聚合页命名来自原文**：需要建聚合概念页时，标题沿用原文里的分类标题（如原文写"医疗"就叫"医疗"）；原文没有分类标题时不建聚合页，靠领域 index 组织。
...
- 逐个核对 entity 页：每页只含该实体自身的信息，并列对象各自独立、无交叉混入。
- 聚合页标题与原文分类标题一致。
