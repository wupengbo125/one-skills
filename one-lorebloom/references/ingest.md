...
扫描收件箱 `raw/` 根目录，把原始资料编译入 `lorebloom/`，随后原文按月归档。
...
2. **扫描收件箱**：只列 `raw/` 根目录（不递归）下的文件。根目录中存在即待摄入，无需标记；收件箱为空则告知用户无待办并结束。

3. **拟定计划**：逐份判断归属领域（life / mindset / technology / stocks / personal / food / english），简短列出拟新建/更新的页面；无匹配领域时先与用户确认，**禁止擅自新建领域**。
...
6. **维护索引与流水**：更新 `lorebloom/<领域>/index.md` 与 `lorebloom/index.md`；向 `lorebloom/log.md`（life 域同时向 `lorebloom/life/log.md`）追加 `## [YYYY-MM-DD] ingest | <主题>`；必要时更新 `onememory/timeline.md`。
