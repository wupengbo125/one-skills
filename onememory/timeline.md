...
- 2026-09-20 00:58 [01a0ba83-ad85-7e10-ae3a-c71cec63072f] one-orca-race：分支名格式定为 <🐎-><随机3位>-<agent>[-<model>]-<短标题>；随机 3 位从"整轮共享一个"改为每位参赛马各生成一个并在同轮去重（用户实测 -a omp omp 两匹马撞同名分支 🐎-154-omp-…，因原实现整轮只随机一次）；短标题先剥掉开头的 [xxx]/【xxx】/(xxx)/（xxx） 标记再截断（原实现把 [Refactor] 计入 6 字符额度，标题被切成 [Refac），含汉字取前 6 个字符、纯英文取前 3 个词；agent/模型/标题统一过新增的 sanitize_ref 清洗 git ref 非法字符（: ? * [ \ ~ ^ 空格等），实测 git check-ref-format 全部合法；SKILL.md 同步
