...
- 2026-09-19 22:17 [01a0b9bf-1380-76c2-862c-9ecf4add49bf] one-orca-race：修复 CodeBuddy 建不出终端（改用 worktree create 返回的 id 定位终端、轮询检测并回车消掉 trust 弹窗后再发 prompt），不带参数直接输出 help，终端标题与分支/worktree 名统一为 🐎-agent名-Issue标题；纠正事实——prime-agent 与 CodeBuddy Code 是两个产品，禁止互指软链
