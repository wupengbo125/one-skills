...
- 2026-09-20 00:49 [01a0ba83-ad85-7e10-ae3a-c71cec63072f] one-orca-race：分支名格式改为 <🐎-><随机3位>-<agent>[-<model>]-<短标题>，随机 3 位 RACE_ID 每轮启动生成一次插在马后面，避免同题多轮分支名撞车；agent 后接用户指定的模型（未指定则不带），区分同名 agent 绑不同模型；标题截断规则定为含汉字取前 6 个汉字、纯英文取前 3 个词（最初的前 6 词/40 字符方案已废弃）；SKILL.md 同步补充分支命名说明
