...
- 2026-09-19 22:28 [01a0b9bf-1380-76c2-862c-9ecf4add49bf] one-orca-race：修复 CodeBuddy 建不出终端（改用 worktree create 返回的 id 定位终端、轮询检测并回车消掉 trust 弹窗后再发 prompt）；裸命令改为直接开跑（删掉无参即 help 的规则，与默认 agent/最新 open issue 冲突）；标题与分支/worktree 名统一为 🐎-agent名-Issue标题（单人参赛不带马）；无 -i 自动取最新 open issue；默认 agent 按用户改定的帮助文本改为选项 -sda/--set-default-agent，用法帮助同步补上缺省行为说明；纠正事实——prime-agent 与 CodeBuddy Code 是两个产品，禁止互指软链
