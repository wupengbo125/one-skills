...
- 2026-09-19 22:28 [01a0b9bf-1380-76c2-862c-9ecf4add49bf] one-orca-race：修复 CodeBuddy 建不出终端（改用 worktree create 返回的 id 定位终端、轮询检测并回车消掉 trust 弹窗后再发 prompt）；不带参数输出 help；标题与分支/worktree 名统一为 🐎-agent名-Issue标题（单人参赛不带马）；无 -i 自动取最新 open issue；新增 race set-default-agent 设默认 agent，用法帮助同步补上子命令与缺省行为说明；纠正事实——prime-agent 与 CodeBuddy Code 是两个产品，禁止互指软链
