...
description: 全新重做 One Free-Me 与海马体记忆系统，规范中文文章命名、扁平化 freewiki 实操手册、支持本地 BM25 检索数据库与双通道记忆
tags:
  - prd
  - one-free-me
  - memory
...
- **freewiki 单层平铺**：不设二级分类目录，专供“AI 操作我电脑的独家实操避坑手册”，外层 `index.md` 统一登记。
...
- `freewiki/`：单层扁平专区，存放独家实操避坑手册，外层 `index.md` 索引每篇手册。
