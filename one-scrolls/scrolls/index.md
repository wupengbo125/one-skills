...
- [Plugin创作手册](./tech/Plugin创作手册.md)
  - **核心解决**：从零创建 Paseo Plugin 的完整流程与踩坑——骨架（`paseo-plugin.json` + 两侧 `contribute` + shared RPC）、zod 双端校验与 `undefined`/`""` patch 约定、数据落 `~/.paseo/plugin-data/<id>/`、RN 表单 StableInput（语音/IME 断字三根因与非受控修法）、reload 验证闭环；含推荐创作顺序。
