...
- [CodeServer网页版VSCode远程改文件部署手册](./tech/CodeServer网页版VSCode远程改文件部署手册.md)
  - **核心解决**：手机/浏览器改 Linux 本机文件的自建方案：code-server 安装避坑（release 资产名无 v 前缀）、密码只用 common_password（systemd user 服务须自行 source exports 注入 $PASSWORD）、只监听 127.0.0.1 由 `tailscale serve --https` 出 HTTPS；附选型对比（File Browser 已归档）与 curl 302/200 验证法；第 10 节给出手机场景正确解——自研 one-files（文件列表+全屏编辑，零依赖）。
