...
- **自动记忆**：修改任何文件后必须自动记录流水至海马体。提交顺序强制：①先提交海马体拿 commit hash；②再提交代码仓库，message 带 `[memory: <hash>]`。commit-msg hook 验证 hash，未通过拒绝提交。
- **自检**：动手前检查 constitution.md、hot-memory.md 是否读了；修改任何文件后检查 memory.md 是否遵守了。没读或没遵守就是失败，立刻补。
